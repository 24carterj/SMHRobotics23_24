...
import org.firstinspires.ftc.teamcode.commands.DriveFC;
...
//@Disabled
@TeleOp(name="DriveFC")
public class DriveLiftFC extends CommandOpMode {
    ...
    private double leftLiftValue = 0;
    private double rightLiftValue = 0;
    private boolean afterLift = false;
    @Override
    public void initialize() {
        ...
        // Lift
        leftLiftValue = driver.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER);
        rightLiftValue = driver.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER);
        if (leftLiftValue > 0.1) {
            schedule(new TwoLiftDown(lifts));
            CommandScheduler.getInstance().run();
            afterLift = true;
        } else if (rightLiftValue > 0.1) {
            schedule(new TwoLiftUp(lifts));
            CommandScheduler.getInstance().run();
            afterLift = true;
        } else {
            if (afterLift) schedule(new InstantCommand(lifts::stop, lifts));
            afterLift = false;
        }
        ...
        drive.setDefaultCommand(new DriveFC(drive, driver::getLeftX, driver::getLeftY, driver::getRightX, drive::getHeading));
    }
}
